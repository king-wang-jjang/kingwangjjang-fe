...
그래픽 이미지, SVG/canvas, 버블, 입자, 이미지 썸네일, 둥근 카드, 채워진 버튼, 색상 배지, 큰 타이포, sticky/fixed stage, 스크롤 연동 장면을 홈에서 사용하지 않는다. 로고는 `logo-single.png`의 왼쪽 사각형·오른쪽 위 원·오른쪽 아래 직각삼각형 배치를 32열·16행의 ASCII 문자로 재현한다. 내부는 `#`로 채우고 홈의 무채색 잉크 토큰을 따른다. 원본 파일은 보존한다. 로고 영역은 데스크톱 132px·모바일 96px 정사각형으로 고정해 움직임이 문서 높이에 영향을 주지 않게 한다. 헤더와 요약도 일반 문서 흐름에 놓는다. 로딩과 오류는 텍스트다.

로고 전체는 7초 주기로 조금씩 떠다니고 평면에서 회전한다. 소개 영역의 마우스 위치를 따라 최대 3px 이동·5도 회전하고, 마우스가 떠나면 기본 위치로 돌아온다. 터치 스크롤에는 반응하지 않는다. `[움직임 켜짐/꺼짐]`과 시스템 모션 감소 설정은 로고와 배경을 함께 제어한다. 로고가 화면 밖이거나 탭이 비활성화되면 애니메이션을 멈춘다. 스크린리더에는 로고 이름과 세 도형만 알리고 개별 문자는 숨긴다. 구현은 `ascii-logo.tsx`와 `ascii-logo.module.css`에서 관리한다.
