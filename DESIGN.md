...
홈 `/`는 텍스트와 ASCII 기호를 기본으로 하며, 시간별 태그 순위 그래프에는 SVG를 사용한다. 아래의 기존 공통 색상·카드·모션 규칙과 충돌하면 `8. Page Blueprints`의 ASCII / Plain Text 홈 절을 우선한다.
...
> 2026-09-10 사용자 수정: 실시간 갱신 상태가 바뀌어도 목록 위치와 페이지 높이가 흔들리지 않도록 메시지 공간을 유지한다. 목록 선택은 밑줄이나 추가 구분선 대신 형광펜 효과로 표시한다.

> 2026-09-10 사용자 수정: 배경은 2D 도형으로 바꾸고, 소개 영역의 마우스 반응형 3D 모델은 프로젝트 로고 원본으로 교체한다.

> 2026-09-14 사용자 수정: 로고는 `public/logo/logo-single.png`를 사용한다. 사용하지 않는 전체형 로고 파일은 삭제하고, 배경 도형은 내부를 ASCII `#` 문자로 채운다.

> 2026-09-14 사용자 추가 수정: 홈의 로고 이미지를 원본 배치에 맞춘 ASCII 문자 로고로 바꾸고, 이전 도형처럼 자동 움직임과 마우스 반응을 적용한다.

> 2026-09-14 사용자 정정: 이전 도넛처럼 실제 3D 좌표·원근·명암을 ASCII 문자로 렌더링한다. 로고를 입체 도형으로 구성하고 회전에 따라 문자와 보이는 면을 갱신한다.

> 2026-09-21 사용자 요청: 홈에 시간별 태그 순위 그래프를 추가한다. 이 데이터 그래프에는 반응형 SVG 선과 점, 태그 구분 색상을 사용한다. 나머지 홈의 ASCII 표현은 유지한다. 시간별 게시글 수 기준 상위 10위와 기존 Activity 순위는 구분해 표시한다.

#### 화면 원칙

한국어 제목과 게시글은 그대로 표시한다. 메뉴, 목록, 조작 표시, 구분선은 텍스트와 ASCII 기호 `[ ] # + - = > / | .`로 구성한다. 소개 영역에는 지정된 프로젝트 로고를 ASCII 문자로 재현한다. 숫자 막대는 `[######....]`이다. Unicode 블록·화살표·그림문자는 장식에 사용하지 않는다.

고정폭 글꼴, 본문 13–14px, 페이지 제목 18–20px, 섹션 제목 17–18px를 사용한다. 제목은 굵기 800과 섹션별 ASCII 구분선으로 강조한다. 태그 순위는 `+---+---+`, 출처별 게시글은 `o---o---o`, 오늘의 인기글은 `/\/\/\/\` 패턴을 반복한다. 헤더는 `: . : .`, 푸터는 `. - . -`로 가볍게 구분한다. 구분선은 한 줄로 표시하고 화면 폭을 넘는 부분은 잘라내며 스크린리더에서 숨긴다. 최대 폭 960px의 문서와 모바일 좌우 16px 여백을 기준으로 한다. 무채색 밝은/어두운 테마를 제공하고, 강조는 굵기·형광펜·대괄호로 나타낸다. 선택한 인기글 제목과 정렬 기준에는 형광펜 표시가 유지되며, 키보드 포커스 윤곽선도 보존한다.
...
헤더는 `마약.kr` 브랜드 바로 오른쪽에 `게시판`, `Top 10` 링크를 같은 줄로 배치한다. 두 링크는 13px 보통 굵기의 고정폭 텍스트와 얇은 밑줄로 표시하고 대괄호·화살표·버튼 테두리를 사용하지 않는다. `전체 보기`는 탐색 영역의 접근성 이름으로 유지한다. 테마·로그인은 오른쪽에 두고 좁은 화면에서는 이 조작 영역만 다음 줄로 내린다. 아래에는 `이 페이지` 레이블과 `[태그]`, `[출처]`, `[인기글]`을 별도 줄에 배치한다. 섹션 링크와 분류 레이블은 muted 색상이며, 클릭 영역은 최소 44px 높이를 유지한다. 게시판·Top 10은 hover와 focus에서 밑줄 대비를 높이고 배경 강조를 사용하지 않는다. 다른 메뉴의 기존 형광펜 효과와 모든 링크의 키보드 포커스 윤곽선은 유지한다. 소개 아래의 전체 콘텐츠 링크, 각 목록 제목 옆의 전체 보기 링크도 탐색 경로로 제공한다. 로딩·빈 결과·오류에서도 전체 페이지로 이동할 수 있어야 한다. 각 섹션으로 이동했을 때 기본 목록 전체가 화면 안에 들어오도록 확인한다. 목록을 내부 스크롤로 가두거나 항목 수를 줄이지 않는다.

그래픽 이미지, SVG/canvas, 버블, 입자, 이미지 썸네일, 둥근 카드, 채워진 버튼, 색상 배지, 큰 타이포, sticky/fixed stage, 스크롤 연동 장면을 홈에서 사용하지 않는다. 로고는 원본 `logo-single.png`의 왼쪽 사각형·오른쪽 위 원·오른쪽 아래 직각삼각형 배치를 유지한다. 사각형과 삼각형에는 두께와 경사진 모서리를, 원에는 구의 곡면을 적용해 하나의 3D 로고로 회전시킨다. 원본 파일은 보존한다.

이전 도넛의 렌더링 원리처럼 3D 좌표를 원근 투영하고 깊이 버퍼로 가려진 면을 처리한다. 빛의 방향·표면 법선·반사광을 계산해 밝기를 ASCII 문자 `.,:-=+*#%@`로 표현한다. 72열·36행의 고정된 텍스트 격자를 최대 24fps로 갱신한다. 무채색 잉크 토큰을 따르며, 로고 영역은 데스크톱 240px·모바일 180px 정사각형이다. 문서 배치는 고정하고 문자만 갱신한다.

로고는 Y축으로 계속 회전하며 X·Z축으로도 기울어진다. 소개 영역의 마우스 위치를 따라 X·Y 회전 각도를 추가하고, 마우스가 떠나면 자동 회전 각도로 부드럽게 돌아온다. 터치 스크롤에는 반응하지 않는다. `[움직임 켜짐/꺼짐]`과 시스템 모션 감소 설정은 로고와 배경을 함께 제어한다. 정지 시 현재 문자 프레임을 유지한다. 로고가 화면 밖이거나 탭이 비활성화되면 프레임 요청을 중지한다. 스크린리더에는 로고 이름과 세 입체 도형만 알리고 개별 문자는 숨긴다. 구현은 `ascii-logo.tsx`, 3D 계산은 `ascii-logo-renderer.ts`, 표시는 `ascii-logo.module.css`에서 관리한다.

아래는 초기 각도의 실제 문자 프레임이다. 회전하면서 실루엣과 음영 문자가 달라진다.
...
홈 배경은 정사각형·원·삼각형의 2D ASCII 도형으로 구성한다. 외곽선 안쪽은 빈 공간 없이 `#`로 채우고, 도형 바깥 여백은 투명하게 유지한다. 문자 모양은 고정되어 있고 평면 위에서 이동·회전한다. 원근, 입체 면, 명암, 깊이에 따른 대비 변화는 적용하지 않는다.

데스크톱에서는 324개, 모바일에서는 162개 도형을 배경 전체에 흩뿌린다. 모든 도형은 화면 크기와 무관하게 5px 문자와 동일한 배율을 사용한다. 도형별 크기 차이나 이동 중 확대·축소는 적용하지 않는다. 각 도형은 서로 다른 속도·방향으로 이동하고 회전한다. 화면 경계를 완전히 벗어나면 반대편에서 이어서 진입한다. 본문 영역에서는 배경 대비를 낮추며 콘텐츠 폭이나 문서 높이를 늘리지 않는다. 홈의 밝은·어두운 잉크 토큰을 함께 사용한다.

배경은 데이터 차트가 아닌 장식이며 스크린리더와 포인터 조작에서 제외한다. `[움직임 켜짐/꺼짐]`과 시스템 모션 감소 설정은 이동·회전을 함께 정지한다. 탭 비활성화나 배경 전체가 화면 밖으로 나가면 프레임 요청을 중지한다. 제목·목록·요약에는 이 효과를 적용하지 않는다. 구현은 `src/sections/home/activity/ascii-shapes.tsx`, 배경 배치는 `ascii-shapes.module.css`에서 관리한다.
...
```text
mayak.kr    board   Top 10                                  [theme] [login]
this page   [tags] [sources] [popular]
---------------------------------------------------------------
...
- 시간별 그래프는 `/boards/issues`의 `hourly_rankings`를 사용한다. 현재 시간대를 포함한 최근 24개 정시 구간에서 실제 게시글 수로 순위를 계산하며, 진행 중인 시간대와 KST 표시를 명시한다. 동률은 태그 이름순이다. 기존 최근 24시간 합계와는 시작 경계가 다르다.
- 6·12·24시간 전환과 태그 선택, 키보드로 조작할 수 있는 시간 선택 막대를 제공한다. 각 시간의 상위 10위에 든 태그 중 표시 기간의 게시글 수 합계가 큰 최대 5개를 기본 비교한다. 집계가 없거나 10위 밖인 구간은 선으로 이어 붙이지 않는다.
...
320px 이상 화면에서 가로 넘침 없음, 시간별 태그 그래프 이외의 이미지/SVG/canvas 없음, 고정된 장면 없음, 태그·출처·정렬·요약·원문·테마·로그인 조작을 확인한다. 시간별 순위 그래프의 기간·태그·시간 선택과 빈 결과·오류를 확인한다. 로고의 ASCII 문자·원본 배치·자동 움직임·마우스 반응과 모션 정지, 화면 밖 정지를 확인한다. 기존 기능 테스트·타입·lint·프로덕션 빌드와 Chromium 검증을 수행한다.
