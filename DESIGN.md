...
헤더의 `[게시판]`, `[Top 10]`은 섹션 메뉴보다 앞에 놓고 16px 굵은 글자, 무채색 사각 테두리, ASCII `>`로 강조한다. 마우스를 올리거나 키보드로 포커스하면 기존 형광펜 효과를 유지한다. 모바일에서는 두 링크를 같은 너비로 한 줄에 배치하고 태그·출처·인기글 메뉴는 다음 줄에 둔다. 소개 아래의 전체 콘텐츠 링크, 각 목록 제목 옆의 전체 보기 링크도 탐색 경로로 제공한다. 로딩·빈 결과·오류에서도 전체 페이지로 이동할 수 있어야 한다. 각 섹션으로 이동했을 때 기본 목록 전체가 화면 안에 들어오도록 확인한다. 목록을 내부 스크롤로 가두거나 항목 수를 줄이지 않는다.
...
```text
mayak.kr    [board] > [Top 10] >    [tags] [sources] [popular]    [theme] [login]
---------------------------------------------------------------
