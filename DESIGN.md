...
한국어 제목과 게시글은 그대로 표시한다. 메뉴, 목록, 조작 표시, 구분선은 텍스트와 ASCII 기호 `[ ] # + - = > / | .`로 구성한다. 소개 영역에는 지정된 프로젝트 로고 이미지를 표시한다. 숫자 막대는 `[######....]`이다. Unicode 블록·화살표·그림문자는 장식에 사용하지 않는다.
...
지정된 프로젝트 로고 외의 그래픽 이미지, SVG/canvas, 버블, 입자, 이미지 썸네일, 둥근 카드, 채워진 버튼, 색상 배지, 큰 타이포, sticky/fixed stage, 스크롤 연동 장면을 홈에서 사용하지 않는다. 로고는 원본의 정사각형 비율·색상·투명 배경을 유지하며, 데스크톱 132px·모바일 96px로 표시한다. 원본 이미지의 1080×1080 크기를 미리 지정해 로딩 시 레이아웃이 움직이지 않도록 한다. 헤더와 요약도 일반 문서 흐름에 놓는다. 로딩과 오류는 텍스트다.
...
배경은 데이터 차트가 아닌 장식이며 스크린리더와 포인터 조작에서 제외한다. `[움직임 켜짐/꺼짐]`과 시스템 모션 감소 설정은 이동·회전을 함께 정지한다. 탭 비활성화나 배경 전체가 화면 밖으로 나가면 프레임 요청을 중지한다. 제목·목록·요약·로고에는 이 효과를 적용하지 않는다. 구현은 `src/sections/home/activity/ascii-shapes.tsx`, 배경 배치는 `ascii-shapes.module.css`에서 관리한다.
...
320px 이상 화면에서 가로 넘침 없음, 지정된 로고 외 이미지/SVG/canvas 없음, 고정된 장면 없음, 태그·출처·정렬·요약·원문·테마·로그인 조작을 확인한다. 기존 기능 테스트·타입·lint·프로덕션 빌드와 Chromium 검증을 수행한다.
