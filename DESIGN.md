...
헤더는 브랜드·테마·로그인 행 아래에 두 종류의 탐색 메뉴를 각각 한 줄씩 배치한다. 첫 줄은 `전체 보기` 레이블과 `[게시판 >]`, `[Top 10 >]`, 둘째 줄은 `이 페이지` 레이블과 `[태그]`, `[출처]`, `[인기글]`이다. 모든 화면 크기에서 이 구분을 유지한다. 링크는 13px 보통 굵기의 ASCII 텍스트로 표시하고 사각 테두리나 채워진 버튼을 사용하지 않는다. 섹션 링크와 분류 레이블은 muted 색상이며, 클릭 영역은 최소 44px 높이를 유지한다. 마우스를 올리거나 키보드로 포커스하면 기존 형광펜 효과와 포커스 윤곽선을 유지한다. 소개 아래의 전체 콘텐츠 링크, 각 목록 제목 옆의 전체 보기 링크도 탐색 경로로 제공한다. 로딩·빈 결과·오류에서도 전체 페이지로 이동할 수 있어야 한다. 각 섹션으로 이동했을 때 기본 목록 전체가 화면 안에 들어오도록 확인한다. 목록을 내부 스크롤로 가두거나 항목 수를 줄이지 않는다.
