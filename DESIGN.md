...
> 2026-09-10 사용자 수정: 배경은 2D 도형으로 바꾸고, 소개 영역의 마우스 반응형 3D 모델은 `public/logo/logo-full.png` 로고 원본으로 교체한다.
...
지정된 프로젝트 로고 외의 그래픽 이미지, SVG/canvas, 버블, 입자, 이미지 썸네일, 둥근 카드, 채워진 버튼, 색상 배지, 큰 타이포, sticky/fixed stage, 스크롤 연동 장면을 홈에서 사용하지 않는다. 로고는 원본 비율과 색상을 유지하며 다크 모드에서도 읽히는 밝은 바탕에 표시한다. 이미지 크기를 미리 지정해 로딩 시 레이아웃이 움직이지 않도록 한다. 헤더와 요약도 일반 문서 흐름에 놓는다. 로딩과 오류는 텍스트다.

홈 배경은 정사각형·원·삼각형의 2D ASCII 외곽선으로 구성한다. 문자 모양은 고정되어 있고 평면 위에서 이동·회전한다. 원근, 입체 면, 명암, 깊이에 따른 대비 변화는 적용하지 않는다.
