...
홈 배경은 정사각형·원·직각삼각형에서 확장한 정육면체·구·직각삼각기둥의 3D ASCII 모델로 구성한다. 눈·코·입·손·다리 등 캐릭터 표현은 사용하지 않는다. 실제 3D 메시를 세 축으로 회전시켜 원근 투영하고, 깊이 버퍼와 면의 조명을 `. : - = + * # % @` 문자 명암으로 표현한다.

데스크톱에서는 324개, 모바일에서는 162개 도형을 배경 전체에 흩뿌린다. 모든 도형은 화면 크기와 무관하게 2.5px 문자와 동일한 배율을 사용한다. 도형별 크기 차이나 이동 중 확대·축소는 적용하지 않는다. 각 모델은 서로 다른 속도·방향으로 이동하고 회전하며, 깊이에 따라 대비만 바뀐다. 화면 경계를 완전히 벗어나면 반대편에서 이어서 진입한다. 본문 영역에서는 배경 대비를 낮추며 콘텐츠 폭이나 문서 높이를 늘리지 않는다. 홈의 밝은·어두운 잉크 토큰을 함께 사용한다.

배경과 소개 영역의 문자 신호는 데이터 차트가 아닌 장식이며 스크린리더와 포인터 조작에서 제외한다. `[움직임 켜짐/꺼짐]`과 시스템 모션 감소 설정은 이동·회전을 함께 정지한다. 탭 비활성화나 배경 전체가 화면 밖으로 나가면 프레임 요청을 중지하고, 개별 도형이 화면 밖에 있으면 문자 표면 계산을 생략한다. 제목·목록·요약에는 이 효과를 적용하지 않는다. 구현은 `src/sections/home/activity/ascii-solids.tsx`, 메시와 투영은 `ascii-solids-renderer.ts`, 배경 배치는 `ascii-solids.module.css`에서 관리한다.
