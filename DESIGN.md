...
320px 이상 화면에서 가로 넘침 없음, 이미지/SVG/canvas 없음, 고정된 장면 없음, 태그·출처·정렬·요약·원문·테마·로그인 조작을 확인한다. 로고의 ASCII 문자·원본 배치·자동 움직임·마우스 반응과 모션 정지, 화면 밖 정지를 확인한다. 기존 기능 테스트·타입·lint·프로덕션 빌드와 Chromium 검증을 수행한다.
