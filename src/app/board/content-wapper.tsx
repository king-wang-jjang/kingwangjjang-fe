...
import useInfiniteScrollablePostList from 'src/hooks/use-infinite-scrollable-post-list';

export const ContentWrapper = () => {
  const pageTheme = useTheme();
  const [postData, setPostData] =
    useState<RealtimePaginationQuery['realtimePagination']>(POSTITEMS);
  const [fiteredPostData, setFiteredPostData] = useState<
  ...
  if (isMobile)
    return (
      boardContentsData?.realtimePagination && (
      ...
      <Grid container spacing={2} margin={0} paddingY="0" position="relative">
        <Grid xs={0} md={3} paddingY="0">
          ...
          <Box width="100%" bgcolor="white" position="sticky" top="73px">
            <Filter
              setFilteredPostData={setFiteredPostData}
              postData={postData}
